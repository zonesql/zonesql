	...
	*/
	...
	function _query($sql,$inputarr)
	{
		...
		if (!is_array($sql) && preg_match('/^\\s*insert/i', $sql)) {
			...
			. $this->identitySQL;
		} elseif (is_array($sql) && isset($sql[3]) && preg_match('/^\\s*insert/i', $sql[3])) {
			$getIdentity = true;
			if (substr($sql[3], strlen($sql[3]) - strlen($this->identitySQL)) != $this->identitySQL) {
				$sql[3] .= (preg_match('/;\\s*$/i', $sql[3]) ? ' ' : '; ')
				. $this->identitySQL;
				$sql[1] = $this->qstr($sql[3]);
			}
		}
		...
		if (is_array($inputarr)) {
			...
			$decl = $this->qstr($decl);
			$resolved_sql = "sp_executesql N{$sql[1]},N$decl,$params";
			if ($this->debug) ADOConnection::outp("<font size=-1>$resolved_sql</font>");
			if ( $doLog ) error_log( "mssql_query,$resolved_sql");
			$rez = mssql_query($resolved_sql, $this->_connectionID);
			
		} else if (is_array($sql)) {
			# PrepareSP()
			if ( $doLog ) error_log( "mssql_execute,$sql[1]");
			$rez = mssql_execute($sql[1]);
			
		} else {
			if ( $doLog ) error_log( "mssql_query,$sql");
